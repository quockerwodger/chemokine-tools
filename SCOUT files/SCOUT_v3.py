...
input_files = open("chemokines_noname.txt").readlines() #opens file used as argument'
...
for sub in kd_all_array: #takes each sub (value?) in the array and appends it to the empty list, removing the \n in the process
    ...
    '''
#Machine learning
train4, test4, train_labels4, test_labels4 = train_test_split(chemokine_array, binding_status, test_size = 0.25, random_state = 42, stratify = binding_status)
...
scores = cross_val_score(clf4, chemokine_array, binding_status, cv = StratifiedKFold(10, shuffle = True, random_state = 42))
